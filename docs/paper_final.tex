...
The main results rely on recall-oriented text-overlap metrics (word, bigram, key-term recall) as proxies for semantic quality; expert annotation under the unified $0$--$3$ rubric has been started but is not complete, so the findings lack independent human validation. The overlap metrics are recall-biased, dividing by $|W_{\text{gold}}|$ and not penalizing irrelevant prediction content, so Llama's higher scores may partly reflect verbosity rather than superior understanding.
