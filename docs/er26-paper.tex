...
\title{Formalization Matters: Large Language Model Output Quality and Consistency Across High-, Semi-, and Low-Formal Tasks}
...
\begin{enumerate}[noitemsep]
  ...
  \item We treat formalization as a \emph{continuous} dimension measured at three points rather than a categorical label: we propose a per-instance metric $F(T)$ along three dimensions and show, via a nested-model ablation, that it predicts LLM output quality \emph{beyond} the categorical level encoding.
\end{enumerate}
\fussy
...
A growing body of work applies LLMs to conceptual modeling itself. C\'{a}mara et al.~\cite{camara2023assessment} assess ChatGPT's ability to produce UML models and report frequent syntactic and semantic defects; Fill et al.~\cite{fill2023conceptual} survey generative-AI use across modeling notations; and H\"{a}rer~\cite{haerer2023conceptual} proposes an LLM-based interpreter that generates models in textual syntaxes such as PlantUML. Recent ER-community work continues this line~\cite{DBLP:conf/er/SilvaMCKP25,Storey2025}. These efforts share a \emph{generation} framing---prompt an LLM to \emph{produce} a model or model fragment---and evaluate ad hoc, typically on a single notation. What is missing, and what we provide, is a controlled measurement of how the \emph{formalization level of the representation} affects output quality and consistency, holding the task family fixed and using metrics that are comparable across levels. Our contribution is thus complementary: rather than proposing another generation method, we characterize where along the formalization spectrum LLM assistance is reliable, information directly useful for deciding when the above methods can be trusted.
...
A challenge is finding metrics applicable uniformly across all three levels. Domain-specific metrics (execution correctness for SQL, inter-anno\-ta\-tor agreement for legal) cannot be compared across levels. We therefore adopt text-overlap metrics against expert gold standards as the \emph{cross-level} metric: while coarser than domain-specific alternatives, this ensures that observed differences reflect the formalization gradient rather than incompatibilities among metrics. For the high-formal level we \emph{additionally} report a rigorous domain-specific metric---SQL execution accuracy (Section~\ref{sec:sql-exec})---to verify that the overlap-based ranking is not an artifact of the proxy; the two agree. We use three recall-oriented overlap measures against the gold standard, where $W$ denotes the set of unique lowercased words---$W_{\text{gold}}$ for words from the gold standard and $W_{\text{pred}}$ for words from the LLM prediction (output):
...
\subsection{High-Formal Execution Accuracy}
\label{sec:sql-exec}

Overlap metrics are coarsest precisely where correctness is objectively decidable---the high-formal level. We therefore complement the overlap scores with execution accuracy. We load the Northwind schema and data into an in-process analytical database (DuckDB) and execute both the gold and the predicted SQL, scoring a prediction correct when its result set exactly matches the gold's (order-insensitive unless the query specifies \texttt{ORDER BY}; the Spider-style execution-accuracy criterion). Five of the 100 gold queries use constructs outside the engine's dialect and are excluded, leaving 95 gradeable tasks ($\times K{=}5$). Table~\ref{tab:sql-exec} reports the results.

\begin{table}[htbp]
\centering
\caption{High-formal SQL execution accuracy (exact result-set match against a populated Northwind database, Spider-style). \emph{runs} is the fraction of predictions that execute without error; \emph{exact} is computed over the 95 gradeable tasks whose gold query executes in the engine.}
\label{tab:sql-exec}
\setlength{\tabcolsep}{6pt}
\begin{tabular}{llrrrr}
\toprule
Model & Metric & Easy & Medium & Hard & Overall \\
\midrule
Mistral & runs   & 97\% & 63\% & 31\% & 62\% \\
        & exact  & 68\% & 53\% & 22\% & \textbf{49\%} \\
\addlinespace
Llama   & runs   & 94\% & 72\% & 55\% & 72\% \\
        & exact  & 67\% & 62\% & 52\% & \textbf{61\%} \\
\bottomrule
\end{tabular}
\end{table}

Execution accuracy corroborates the overlap-based analysis on an objective metric. Llama outperforms Mistral (61\% vs.\ 49\% exact), the same direction as the recall and $F_1$ comparisons, confirming that the model ranking is not an artifact of the overlap proxy. Two points stand out. First, accuracy is far from perfect even at the formal level: fewer than two-thirds of queries return the correct result, and 27--38\% of queries that \emph{execute} still return a wrong result set---reinforcing that formalization raises the ceiling but does not guarantee correctness, so verification remains necessary. Second, accuracy declines monotonically with query difficulty (easy $>$ medium $>$ hard for both models), an execution-grounded instance of the within-domain gradient examined next.
...
The 100 SQL tasks are tagged by difficulty: easy (20 tasks), medium (55 tasks), and hard (25 tasks). These difficulty levels represent a within-domain formalization gradient: easy queries have a more constrained output space ($C(T)$ closer to 1) than hard multi-table queries where multiple valid join strategies exist. Comparing word overlap across difficulty tags, we observe a decline from $.831$ (easy) to $.798$ (medium) to $.708$ (hard), mirroring the cross-level pattern within a single domain. Execution accuracy shows the same monotonic within-domain gradient (Table~\ref{tab:sql-exec}: Mistral $68\%\!\to\!53\%\!\to\!22\%$; Llama $67\%\!\to\!62\%\!\to\!52\%$), so the gradient is not an artifact of the overlap metric.
...
\subsection{Robustness to Sampling Temperature}
\label{sec:temp-robustness}

Because the variance paradox could be an artifact of the default sampling temperature ($T\!=\!0.7$), we re-ran the full benchmark---all three levels, both models---at $T \in \{0, 0.3, 0.7, 1.0\}$ ($K\!=\!1$ for greedy $T\!=\!0$, $K\!=\!5$ otherwise). Table~\ref{tab:temp-sweep} reports per-level within-task (across-run) variance of word overlap at each temperature. The paradoxical ordering---\emph{more} formal tasks show \emph{higher} run-to-run variance---holds at every sampled temperature for both models. As expected, absolute variance grows with temperature (and is zero at $T\!=\!0$, where greedy decoding is deterministic), but the high $>$ semi $>$ low ordering is invariant. The quality gradient is likewise stable: the dominant high$\,\to\,$semi drop persists at all temperatures (high word overlap $.76$--$.80$ vs.\ semi $.18$--$.25$), with mean quality varying by less than $.02$ across the temperature range. The paper's two central findings are therefore not sampling-regime artifacts.

\begin{table}[htbp]
\centering
\caption{Within-task (across-run) variance of word overlap by level and temperature (Mistral / Llama). The variance paradox (high $>$ semi $>$ low) holds at every $T>0$ for both models; $T\!=\!0$ is deterministic so variance is zero by construction.}
\label{tab:temp-sweep}
\setlength{\tabcolsep}{6pt}
\begin{tabular}{lccc}
\toprule
$T$ & Var$_{\text{High}}$ (M/L) & Var$_{\text{Semi}}$ (M/L) & Var$_{\text{Low}}$ (M/L) \\
\midrule
0.0 & .0000 / .0000 & .0000 / .0000 & .0000 / .0000 \\
0.3 & .0021 / .0035 & .0010 / .0023 & .0004 / .0005 \\
0.7 & .0039 / .0068 & .0014 / .0020 & .0005 / .0006 \\
1.0 & .0051 / .0071 & .0015 / .0031 & .0005 / .0004 \\
\bottomrule
\end{tabular}
\end{table}
...
The refutation of H2 is important to understand: output variance \textit{decreases} rather than increases with decreasing formalization. High-formal SQL tasks produced bimodal outcomes, where outputs were either close to the gold standard or substantially incorrect, whereas low-formal management tasks produced more consistently moderate outputs even when wording differed. Formalization therefore raises expected quality while widening the variance distribution, implying that schema-driven LLM applications should not assume stable correctness. Our data establish the need for verification (the bimodal outcome distribution and the $49$--$61\%$ execution accuracy) but do not by themselves establish that a particular mitigation is sufficient; verification against ground truth and, as a design heuristic warranting further study, retry-and-select strategies are therefore advisable rather than proven remedies.
...
Both evaluated models are $7$--$8$B parameters. Generalization of the findings to $70$B+ models, where better instruction following and richer internal representations could alter the formalization gradient, is not warranted. The variance paradox is, however, robust to sampling temperature: a full sweep over $T \in \{0, 0.3, 0.7, 1.0\}$ (Section~\ref{sec:temp-robustness}) reproduces the within-task variance ordering (high $>$ semi $>$ low) at every sampled temperature for both models, and the dominant high$\,\to\,$semi quality drop persists throughout.
