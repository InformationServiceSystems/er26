#!/bin/bash
# Wrapper script for running all Mistral-7B experiments on HPC cluster
...
echo ""
echo "=== Running high_formal (Mistral-7B, K=5) ==="
...
echo ""
echo "=== Running semi_formal (Mistral-7B, K=5) ==="
python3 scripts/run_semi_formal_local.py \
    --model "$MISTRAL" \
    --output "data/results_raw/semi_formal_mistral_7b.jsonl" \
    --num_runs 5 \
    2>&1
echo "semi_formal exit code: $?"

echo ""
echo "=== Running low_formal (Mistral-7B, K=5) ==="
python3 scripts/run_low_formal_local.py \
    --model "$MISTRAL" \
    --output "data/results_raw/low_formal_mistral_7b.jsonl" \
    --num_runs 5 \
    2>&1
